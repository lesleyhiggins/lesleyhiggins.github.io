...
The DMO standard layout is boring, but it's also a canvas that you can build on.
...
Once you add the Related List Enrichment as a Dynamic Related List on the Contact, Lead, or Account record, make sure to include the "id" field in the list. This creates a clickable link to the DMO record — and this is the key to making DMOs accessible to your sales and service reps. When they click the link, the DMO record (with your custom LWC) opens in a new tab.

### How Related List Enrichments Work

A Related List Enrichment creates a lookup relationship from a DMO to a CRM object (like Contact, Lead, or Account). Once that relationship exists, you can add a Related List of DMO records directly onto a Contact or Account record page right in Service Cloud or Sales Cloud, alongside Cases, Opportunities, and Activities.
...
- **Consider Copy Field Enrichments for single-value data.** If you only need one metric from Data 360 (like a Lifetime Value score or a pet count), use a Copy Field Enrichment instead. It syncs the value initially and keeps it updated via incremental sync - no per-page-load query cost.
...
The combination of custom LWCs and Data 360 Enrichments creates multiple pathways to surface your data:

### **LWCs on DMO Pages**
- **Credit impact**: Minimal 
- **Best for**: Rich data visualization, image rendering, data quality monitoring
- **Key benefit**: Transform raw DMO data into actionable insights for power users

### **Related List Enrichments**
- **Credit impact**: Moderate
- **Best for**: Displaying multiple related records, engagement timelines, transaction history
- **Key benefit**: Deliver Data 360 insights directly in CRM where users already work

### **Copy Field Enrichments**  
- **Credit impact**: Initial full sync + ongoing incremental sync costs
- **Best for**: Single calculated values, scores, flags, or summary metrics
- **Key benefit**: No per-page-load query costs, data stored directly on CRM records
